    ...
    if (playlistIdMatch) {
          ...
          if (initialPlaylistResponse.ok) {
            ...
            allTracks.forEach((track, index) => {
              ...
              metadataResult.innerHTML += `
                  <h3>[${index + 1} / ${allTracks.length}]</h3>
                    ...
                    <br>
                    <button type="button" class="download-button" data-track-id="${
                      index + 1
                    }">Scarica ${title}</button>
                    <br>
                    <br>`;
            });
            ...
            downloadButtons.forEach((button) => {
              button.addEventListener("click", () => {
                ...
                const artist = selectedTrack.artists
                  .map((artist) => artist.name)
                  .join(", ");
        ...
        if (tokenResponse.ok) {
          ...
          if (response.ok) {
            ...
            metadataResult.innerHTML += `
              ...
              <p><strong>Release Date:</strong> ${data.album.release_date}</p>
              <button type="button" id="downloadButton">Scarica Il Brano</button>`;
      ...
      try {
        ...
        if (tokenResponse.ok) {
          ...
          // Fetch the initial album data to get the total number of tracks
          ...
          if (initialAlbumResponse.ok) {
            ...
            allTracks.forEach((track, index) => {
              ...
              metadataResult.innerHTML += `
                <h3>[${index + 1} / ${allTracks.length}]</h3>
                <p id="title"><strong>Name:</strong> ${title}</p>
                <p id="artist"><strong>Artist(s):</strong> ${artist}</p>
                <button type="button" class="download-button" data-track-id="${
                  index + 1
                }">Scarica ${title}</button>
                <br>
            ...
            // Add an event handler to the download buttons
            const downloadButtons =
              document.querySelectorAll(".download-button");
            ...
            downloadButtons.forEach((button) => {
              button.addEventListener("click", () => {
                const trackId = button.getAttribute("data-track-id");
                const selectedTrack = allTracks[trackId - 1];

                // Now you can use selectedTrack to download the specific song
                const artist = selectedTrack.artists
                  .map((artist) => artist.name)
                  .join(", ");
                const title = selectedTrack.name;
                ...
                // Execute the download for the specific song
                // You can use the query to search for the song on YouTube and get the download link
                fetch(
                  `https://www.googleapis.com/youtube/v3/search?key=${API_KEY}&q=${query}&type=video&part=snippet`
                )
                  .then((response) => response.json())
